...
class Bridge(object):
    ...
    async def makecid(*args, **kwargs):
        ...
        result = r8_sc.makeCid(cus_hash, "0x" + owneraddr, descr_hash, 
    ...
    async def changeowner(*args, **kwargs):
        kwargs = json.loads(kwargs.get("message"))
        cid = int(kwargs.get("cid"))
        new_owner = "0x" + kwargs.get("new_owner")
    ...
    async def sellcontent(*args, **kwargs):
        ...
        cid = int(kwargs.get("cid"))
        buyer_address = "0x" + kwargs.get("buyer_address")
        access_string = kwargs.get("access_string")
        seller_public_key = kwargs.get("seller_public_key")
    ...
    async def add_review(*args, **kwargs):
        ...
        result = r8_sc.addReview(cid, buyer_address, review_hash)
    ...
    async def get_buyer_offers(*args, **kwargs):
        logging.debug("\n\n [+] -- Get buyer offers debugging. ")
        kwargs = json.loads(kwargs.get("message"))
        handler = get_contract_handler()
        res = []
        buyer_address = "0x" + str(kwargs.get("buyer_address"))
    ...
    async def get_offer(*args, **kwargs):
        ...
        offer_id = handler.CidBuyerIdToOfferId(cid, 
            R8_Ethereum.to_checksum_address(buyer_address))
