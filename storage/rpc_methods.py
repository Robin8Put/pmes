...
class StorageTable(mongo.Table):
	...
	def __init__(self, *args, **kwargs):
		...
		self.database = self.client[settings.DBNAME]
		self.collection = self.database[settings.ACCOUNTS]
	...
	async def get_contents_dict(self):
		contents = {}
		for i in list(settings.bridges.keys()):
			contents[i] = []
		return contents


	async def retrieve_contents(self, account):
		contents = await self.get_contents_dict()
		for coinid in contents:
			database = client[coinid]
			content_collection = database[settings.CONTENT]
			async for content in content_collection.find({"owner":account["public_key"]}):
				yield {i:content[i] for i in content if i != "_id"}
